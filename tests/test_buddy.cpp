...
public:
    void lock() noexcept {
        uint16_t t = next_.fetch_add(1, relaxed);
        while (now_.load(acquire) != t)
            cpu_relax();
    }
...
public:
    explicit irq_lock_guard(irq_spinlock& l) noexcept
        : l_(l)
        , f_(l.lock()) {}
    ~irq_lock_guard() noexcept { l_.unlock(f_); }
};
template <typename... Args>
void print(const char*, Args...) noexcept {}
} // namespace kernel
...
uintptr_t buddy_allocator::alloc(uint32_t order) noexcept {
    ...
    uint32_t cur = order;
    while (cur <= MAX_ORDER && !free_lists_[cur])
        cur++;
    if (cur > MAX_ORDER) return 0;
    free_block* block = free_lists_[cur];
    free_lists_[cur] = block->next;
...
void buddy_allocator::free(uintptr_t addr, uint32_t order) noexcept {
    ...
    while (order < MAX_ORDER) {
        ...
        while (*pp) {
            if (reinterpret_cast<uintptr_t>(*pp) == ba) {
                *pp = (*pp)->next;
                break;
            }
            pp = &(*pp)->next;
