...
extern "C" void svc_handler(uint32_t num, uint32_t a1, uint32_t a2, uint32_t a3,
                             uint32_t a4, uint32_t a5, uint32_t a6) noexcept {
...
extern "C" void data_abort_handler(uint32_t fault_addr, uint32_t dfsr) noexcept {
    ...
    if (is_translation) {
        ...
        if (kernel::memory::vmm::handle_page_fault(fault_addr, error))
            return;
    }

    kernel::print("Data Abort: addr=0x{x} DFSR=0x{x} status=0x{x}\n",
                  reinterpret_cast<void*>(static_cast<uintptr_t>(fault_addr)),
                  reinterpret_cast<void*>(static_cast<uintptr_t>(dfsr)),
                  reinterpret_cast<void*>(static_cast<uintptr_t>(status)));
...
extern "C" void prefetch_abort_handler(uint32_t fault_addr, uint32_t ifsr) noexcept {
    ...
    if (is_translation) {
        if (kernel::memory::vmm::handle_page_fault(fault_addr, 0))
            return;
    }
