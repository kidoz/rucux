...
static void put(fmt_ctx& c, char ch) {
    if (c.limit == 0 || c.pos < c.limit - 1) c.buf[c.pos] = ch;
    c.pos++;
}
...
static void emit_number(fmt_ctx& c, const char* prefix, int plen, char* digits, int dlen, int width, int prec,
                        char padch, bool left) {
...
static void emit_double(fmt_ctx& c, double val, int width, int prec, char padch, bool left, bool plus, bool space) {
    if (prec < 0) prec = 6;
    ...
    // Format integer part
    char itmp[24];
    int ilen = fmt_uint(itmp, ipart, 10, false);
...
static int do_format(fmt_ctx& c, const char* fmt, va_list ap) {
    while (*fmt) {
        ...
        if (*fmt == 'l') {
            fmt++;
            length = (*fmt == 'l') ? (fmt++, L_LL) : L_L;
        } else if (*fmt == 'z') {
            length = L_Z;
            fmt++;
        } else if (*fmt == 'h') {
            fmt++;
            length = (*fmt == 'h') ? (fmt++, L_HH) : L_H;
        } else if (*fmt == 'j' || *fmt == 't') {
            length = L_L;
            fmt++;
        } // treat as long
